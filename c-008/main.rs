...
fn greatest_prod_of(limit: usize, list: &[i64]) -> i64 {
	let mut result: i64 = 0;
	for i in 0..(list.len()-limit) {
    	let mut prod: i64 = 1;
		for productor in &list[i..(i + limit)] {
			prod = *productor * prod;
		}		
		result = cmp::max(prod, result);
	}
	return result;
}

fn main() {
	println!("{}", greatest_prod_of(13, &PROBLEM_DATA));
}
