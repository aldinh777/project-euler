function collatzRule(num: number) {
  return num % 2 === 0 ? num / 2 : num * 3 + 1;
}

function fillChain(num: number, map: Map<number, number>) {
  const stack = [num];
  let collatz = num;
...
function largestChain(limit: number) {
  const chainLength = new Map();
  chainLength.set(1, 1);
  for (let i = 2; i <= limit; i++) {
    fillChain(i, chainLength);
  }
  const result = Array.from(chainLength).reduce((a, b) =>
    a[1] > b[1] ? a : b,
  );
  return result[0];
}
